...
def login_handler(event, context):
    ...
    try:
        ...
        if not email or not password:
            return error_response('Email and password required', 400, 'BAD_REQUEST', event)
        ...
        if not player or not verify_password(password, player['password_hash']):
            return error_response('Invalid credentials', 401, 'UNAUTHORIZED', event)
        
        if player['status'] == 'dead':
            return error_response('Account is inactive. Contact administrator.', 403, 'ACCOUNT_INACTIVE', event)
        ...
        cookie_value = f"auth_token={token}; Path=/; HttpOnly; Secure; SameSite=None; Max-Age=604800"
...
def register_handler(event, context):
    ...
    try:
        ...
        if not all([nickname, email, password, faction]):
            return error_response('All fields required', 400, 'BAD_REQUEST', event)
        ...
        with get_db() as conn:
            with conn.cursor() as cursor:
                ...
                if existing:
                    return error_response('Nickname or email already exists', 409, 'CONFLICT', event)
...
def me_handler(event, context):
    ...
    try:
        ...
        if not player:
            return error_response('Authentication required', 401, 'UNAUTHORIZED', event)
        ...
        if not player_data:
            return error_response('Player not found', 404, 'NOT_FOUND', event)
...
def logout_handler(event, context):
    ...
    cookie_value = "auth_token=; Path=/; HttpOnly; Secure; SameSite=None; Max-Age=0"
    headers = cors_headers(event)
    headers['Set-Cookie'] = cookie_value
