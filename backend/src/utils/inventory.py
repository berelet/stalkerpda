...
def get_backpack_items(player_id: str, conn) -> Tuple[List[Dict], int]:
    ...
    cursor.execute("""
        ...
        WHERE pi.player_id = %s AND pi.item_type = 'consumable' AND pi.slot_type = 'backpack'
    """, (player_id,))
    ...
    for item in consumable_items:
        backpack.append({
            ...
            'isStackable': bool(item['is_stackable']),
            'isPhysical': bool(item.get('is_physical')),
            'extraLives': item['extra_lives'],
            'imageUrl': item['image_url'],
            'description': item['description'],
            'basePrice': float(item['base_price'])
        })
