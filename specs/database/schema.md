...
Based on Game Mechanics FINAL-SPEC.md
...
CREATE TABLE artifact_types (
    ...
    radiation_resist INT DEFAULT 0,  -- percentage
    other_effects JSON,
    
    -- Metadata
    created_by VARCHAR(36),  -- GM who created this type
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
...
CREATE TABLE equipment_types (
    ...
    radiation_resist INT DEFAULT 0,  -- percentage
    radiation_removal INT DEFAULT 0, -- percentage (for consumables)
    
    -- Economy
    base_price DECIMAL(10, 2) NOT NULL,
    
    -- Limits
    max_slots INT DEFAULT 1,  -- armor=1, rings=3
...
CREATE TABLE radiation_zones (
    ...
    -- Radiation
    radiation_level INT NOT NULL,  -- 10-100 per 5 minutes
    ...
    SPATIAL INDEX idx_center (center_lat, center_lng)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
```
...
CREATE TABLE control_points (
    ...
    latitude DECIMAL(10, 8) NOT NULL,
    longitude DECIMAL(11, 8) NOT NULL,
    capture_radius INT DEFAULT 2,  -- meters
...
CREATE TABLE contracts (
    ...
    delivery_to VARCHAR(36),           -- delivery
    delivery_item_id VARCHAR(36),      -- delivery
    destination_lat DECIMAL(10, 8),    -- escort, delivery
    destination_lng DECIMAL(11, 8),    -- escort, delivery
    ...
    -- Timestamps
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    available_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,  -- Time-gated availability
...
CREATE TABLE shop_inventory (
    ...
    item_id VARCHAR(36),  -- Reference to equipment_types or NULL for physical items
    ...
    is_physical BOOLEAN DEFAULT FALSE,  -- Physical items (ammo, food)
...
CREATE TABLE purchase_transactions (
    ...
    items JSON NOT NULL,  -- Array of {item_id, quantity, price}
...
CREATE TABLE looting_events (
    ...
    equipment_stolen JSON,  -- Array of equipment IDs
    artifacts_stolen JSON,  -- Array of artifact IDs
    ...
    -- Ensure one loot per death
    UNIQUE KEY unique_victim_loot (victim_id, created_at)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
```
...
- Player locations: Spatial index for proximity queries
- Artifacts: Spatial index + state index
- Contracts: Status, faction, availability time
- Events: Type, player, timestamp
- Location history: Player + time for GM tracking

**Unique constraints:**
- Player nickname, email, QR code
- One loot per death (victim + timestamp)

---

## Data Persistence Rules

**Between Games (Persistent):**
- ✅ Balance
- ✅ Reputation
- ✅ Equipment
- ✅ Total stats (kills, deaths, etc.)

**Per Game (Reset):**
- ❌ Lives (reset to default)
- ❌ Radiation (reset to 0)
- ❌ Artifacts (cleared)
- ❌ Contracts (cleared or expired)
- ❌ Zone control (cleared)
